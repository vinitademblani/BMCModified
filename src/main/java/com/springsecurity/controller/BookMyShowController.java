...
public class BookMyShowController {
	...
	// find all the cities.This method can be used to dynamically populate the
	// date in the dropdown of cities
	@GetMapping("/cities")
	...
	// This can be used when the user selects specific city from dropdown then
	// this method will populate all the movies in that city
	...
	public ResponseEntity<Movie> getMovieInCity(@PathVariable("cityId") Long cityId,
	...
	public ResponseEntity<String> getMovieNameByMovieId(@PathVariable("cityId") Long cityId,
			@PathVariable("movieId") Long movieId) {
		if (!cityService.existsById(cityId))
			throw new ResourceNotFoundException("No city found with cityId as " + cityId);
	...
	public ResponseEntity<List<Theatre>> getAllTheatresByMovieId(@PathVariable("cityId") Long cityId,
			@PathVariable("movieId") Long movieId) {
		if (!cityService.existsById(cityId))
			throw new ResourceNotFoundException("No city found with cityId as " + cityId);
		...
		if (allTheatresOfMovie.isEmpty())
			throw new ResourceNotFoundException(
					"No theatres found with  cityId as " + cityId + " with movieId as " + movieId);
	...
	public ResponseEntity<List<Show>> getAllShowsOfMovieInTheatre(@PathVariable("cityId") Long cityId,
			@PathVariable("movieId") Long movieId, @PathVariable("theatreId") Long theatreId) {
		if (!cityService.existsById(cityId))
			throw new ResourceNotFoundException("No city found with cityId as " + cityId);
		...
		if (!showsOfMovie.isPresent())
			throw new ResourceNotFoundException("No shows found with  of movie with movie id as  as " + movieId
					+ " in theatre with theatre id  as " + theatreId);
	...
	@GetMapping(value="/cities/{cityId}/shows/movieImage/{movieImage}",produces = MediaType.IMAGE_JPEG_VALUE)
	public ResponseEntity<byte[]> getMovieImages(@PathVariable("cityId") Long cityId,
			@PathVariable("movieImage") String movieImage) {
		byte[] moveiImages = showService.getImagesByCityId(cityId, movieImage);
		return new ResponseEntity<byte[]>(moveiImages, HttpStatus.CREATED);
	}

	@GetMapping(value="/cities/{cityId}/shows/movieImage",produces = MediaType.IMAGE_JPEG_VALUE)
	public ResponseEntity<byte[]> getMovieImages(@PathVariable("cityId") Long cityId) {
		byte[] moveiImages = showService.getAllMovieImageByCityId(cityId);
		return new ResponseEntity<byte[]>(moveiImages, HttpStatus.CREATED);
	}
}
